...
/**
 * Get global package path using npm root -g
 */
...
/**
 * Check if package is available locally or globally
 * @param {string} packageName 
 * @param {string} projectPath 
 */
function checkPackageAvailability(packageName, projectPath) {
    // Check local
    const localPath = path.join(projectPath, 'node_modules', packageName);
    if (fs.existsSync(localPath)) {
        return { type: 'local', path: localPath };
    }

    // Check global
    const globalPath = getGlobalPackagePath(packageName);
    if (globalPath) {
