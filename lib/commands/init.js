    ...
    .action(async (projectName) => {
        ...
        try {
            ...
            const packageJson = {
                ...
                "dependencies": {
                    "estreui": `^${rootPackage.dependencies.estreui.replace('^', '')}`
                },
                "devDependencies": {
                    "create-estreui": `^${rootPackage.version}`
                }
            };
            fs.writeFileSync(
            ...
            // 4. Install Dependencies & Determine Core Path
            let corePath;
            const globalEstreuiPath = require('../utils').getGlobalPackagePath('estreui');
            let useLocalInstall = true;

            if (globalEstreuiPath) {
                const { installChoice } = await inquirer.prompt([
                    {
                        type: 'confirm',
                        name: 'installChoice',
                        message: 'Global EstreUI package detected. Do you want to install it locally in this project?',
                        default: true
                    }
                ]);
                useLocalInstall = installChoice;
            }

            if (useLocalInstall) {
                const installed = require('../utils').installDependencies(projectPath);
                if (installed) {
                    corePath = path.join(projectPath, 'node_modules/estreui');
                } else {
                    console.warn('⚠ Local installation failed. Trying to fall back to global or CLI dependencies...');
                }
            } else {
                console.log('Skipping local installation. Using global EstreUI package.');
                corePath = globalEstreuiPath;
            }

            // Fallback if still not found (e.g. local install failed or not chosen but global missing/invalid)
            if (!corePath || !fs.existsSync(corePath)) {
                // Try CLI's own node_modules (if installed as dependency of CLI)
                try {
                    // Resolve from CLI's location
            ...
            if (!fs.existsSync(corePath)) {
                console.error('❌ Error: Could not locate EstreUI core package.');
                console.error('Please install it globally (npm i -g estreui) or locally.');
                return;
            }

            console.log(`Using core assets from: ${corePath}`);
            ...
            if (fs.existsSync(corePath)) {
                ...
                // 2a-1. Fallback favicon — estreui core does not ship favicon.ico yet,
                // but index.html and the Service Worker cache list reference it.
                // Copy the bundled neutral default when the core did not provide one.
                const faviconDest = path.join(projectPath, 'favicon.ico');
                if (!fs.existsSync(faviconDest)) {
                    const fallbackFavicon = path.join(__dirname, '../../assets/favicon.ico');
                    if (fs.existsSync(fallbackFavicon)) {
                        await copyFile(fallbackFavicon, faviconDest);
                    }
                }

                // 2b. Verify Service Worker cache list against copied files.
                // cache.addAll() rejects entirely on a single 404, silently breaking PWA
                // install — so any missing reference must be surfaced at init time.
                const swPath = path.join(projectPath, 'serviceWorker.js');
                if (fs.existsSync(swPath)) {
                    const swSource = fs.readFileSync(swPath, 'utf8');
                    const cacheRefs = [...swSource.matchAll(/"\.\/([^"]+)"/g)].map(m => m[1]).filter(Boolean);
                    const missingRefs = cacheRefs.filter(ref => !fs.existsSync(path.join(projectPath, ref)));
                    if (missingRefs.length) {
                        console.warn(`⚠ Service Worker cache list references missing file(s): ${missingRefs.join(', ')}`);
                        console.warn('  PWA install will fail until these exist or are removed from serviceWorker.js.');
                    }
                }
                ...
                for (const lib of libs) {
                    ...
                    if (fs.existsSync(destLibPath)) {
                        ...
                        if (libPath && fs.existsSync(libPath)) {
                            ...
                            // Fallback 3: Try to resolve from CLI dependencies
                            if (!libPath || !fs.existsSync(libPath)) {
                                try {
                                    if (lib === 'jquery') {
                                        libPath = require.resolve('jquery/dist/jquery.js');
                                    } else {
                                        // Try main entry point
                                        const pkgPath = require.resolve(`${lib}/package.json`);
                                        const pkgDir = path.dirname(pkgPath);
                                        // Most of these libs might be single files or have a main field
                                        // Let's try to guess or use main
                                        const pkg = require(pkgPath);
                                        if (pkg.main) {
                                            libPath = path.resolve(pkgDir, pkg.main);
                                        } else {
                                            libPath = path.join(pkgDir, `${lib}.js`);
                                        }
                                    }
                                    console.log(`Found ${lib} in CLI dependencies: ${libPath}`);
                                } catch (e) {
                                    // Ignore
                                }
                            }
                ...
                // 7. Copy essential root files (e.g., .htaccess, .gitignore, webmanifest.json, favicon.ico)
                const rootFiles = ['.htaccess', '.gitignore', 'webmanifest.json', 'favicon.ico'];
                ...
                // 7.1 Ensure .gitignore exists and includes node_modules
                ...
                if (!gitignoreContent.includes('node_modules')) {
                    ...
                    // If file didn't exist and wasn't copied (e.g. not in core), create it
