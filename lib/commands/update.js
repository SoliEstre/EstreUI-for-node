...
program
  ...
  try {
    ...
    // ① Resolve core package location (fallback to local source)
    let corePath = path.resolve('node_modules', 'estreui');
    if (!fs.existsSync(corePath)) {
      // Fallback: try to find it relative to this script (useful for local testing/dev)
      // __dirname is .../lib/commands
